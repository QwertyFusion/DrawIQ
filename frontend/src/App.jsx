...
function App() {
  ...
  const colors = [
    '#FFFFFF', '#FF0000', '#00FF00', '#009CFF', 
    '#FFFF00', '#FF69B4', '#8B4513'
  ]
  ...
  const handleToolbarMouseDown = (e) => {
    ...
    if (e.target === toolbarRef.current || toolbarRef.current.contains(e.target)) {
    ...
    <div className="w-screen h-screen overflow-hidden bg-gray-900">
      <div 
        ...
        className={`fixed flex items-center gap-4 p-4 bg-gray-800 rounded-lg shadow-lg border border-gray-700 transition-none select-none ${
          showColorPicker ? 'cursor-default' : 'cursor-grab active:cursor-grabbing'
        }`}
        style={{
          left: '0px',
          top: '0px',
      ...
      >
        ...
        <div className="flex items-center gap-2 pr-4 border-r border-gray-600">
          <img src="/drawiq-logo.svg" alt="DrawIQ" className="w-8 h-8" />
          ...
          <div className="flex gap-2 items-center">
            {colors.map((c) => (
              <div
                ...
                className={`w-8 h-8 rounded-full cursor-pointer transition-transform hover:scale-110 ${
                  color === c ? 'ring-2 ring-white' : 'ring-1 ring-gray-600'
                }`}
            ...
            <button
              className={`w-8 h-8 rounded-full flex items-center justify-center bg-gradient-to-br from-red-500 via-green-500 to-blue-500 transition-transform hover:scale-110 ${
                showColorPicker ? 'ring-2 ring-white' : 'ring-1 ring-gray-600'
              }`}
        ...
        <div className="flex gap-2 items-center">
          <button 
            className={`p-2 rounded-md transition-colors ${
              mode === 'draw' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-200 hover:bg-gray-600'
            }`}
          ...
          <button 
            className={`p-2 rounded-md transition-colors ${
              mode === 'rect' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-200 hover:bg-gray-600'
            }`}
          ...
          <button 
            className={`p-2 rounded-md transition-colors ${
              mode === 'circle' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-200 hover:bg-gray-600'
            }`}
          ...
          <button 
            className={`p-2 rounded-md transition-colors ${
              mode === 'line' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-200 hover:bg-gray-600'
            }`}
          ...
          <button 
            className={`p-2 rounded-md transition-colors ${
              mode === 'erase' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-200 hover:bg-gray-600'
            }`}
          ...
          <button 
            ...
            className={`p-2 rounded-md transition-colors ${
              historyStep <= 0 ? 'bg-gray-600 text-gray-400 cursor-not-allowed' : 'bg-gray-700 text-gray-200 hover:bg-gray-600'
            }`}
            title="Undo"
          >
            <Undo size={20} />
